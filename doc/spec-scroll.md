...
| R3 | Command palette scrolls (client-owned). Client computes its palette scroll view with R1 (`keep_selection_visible=true` on selection move; false on explicit scroll), reports the **windowed** rows, the **absolute** `selected`, `first_visible`, and the resolved `ScrollbarMetrics` in `PaletteReport`; `shell_view` projects them into `PaletteProjection` verbatim; `paint_palette` renders the window and the gutter thumb from the reported metrics, highlighting `selected - first_visible`; content width is unchanged whether or not a thumb shows. **Wire scope:** `PaletteReport` is a client→server call-parameter struct (not a serialized snapshot section), so the new fields are passed in-process, not encoded; `PaletteProjection` is part of `ShellViewState` — add its new `first_visible` + `ScrollbarMetrics` fields to the `ShellViewState`/`PaletteProjection` protocol codec with a round-trip test (the rows + selected already cross the wire). | `include/ssg/palette.h`, `include/ssg/ui_layout.h`, `src/runtime/snapshot.cpp`, `src/render.cpp`, `apps/ssg_main.cpp`, `src/protocol.cpp` (+ `PaletteProjection` round-trip test), `tests/test_render.cpp`, `tests/test_ssg_app.cpp`, `tests/test_protocol.cpp` | app unit: with N ranked candidates and a pane of `rows` < N, the client's reported window is exactly the R1 window around the absolute `selected`, and shrinking N by typing so `N <= rows` hides the thumb without changing `content` width. render: a projection with more rows than fit draws exactly the windowed rows + a correctly sized thumb, highlighting `selected - first_visible`; `selected` stays visible after windowing. protocol: `PaletteProjection` round-trips the new fields. PTY: type to produce a long list, arrow past the fold, the list scrolls and the selected row stays on screen. |
