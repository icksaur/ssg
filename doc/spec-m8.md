...
**Focus follows the click (M8-F).** So the keyboard tracks what the mouse acts on,
the acted-on-surface commands own the focus move, entirely server-side:
`cursor.set_position` and `select.set_range` focus the editor; `tab.activate`
focuses the editor; `tree.select` focuses the panel. Safety is by command/enum
gating, not by these being keyboard-unreachable: no keymap binding dispatches them
(keyboard caret motion and tab switching are *different* commands), so ordinary
keyboard use never moves focus; and where one is reachable another way (e.g.
`cursor.set_position` is a palette candidate), focusing the surface it acts on is
the correct outcome regardless. Because these run in the same transaction as the
click's action and the app re-adopts `shell.focus` on the next event, a click both
acts and moves the keyboard's active surface. See M8-F.
...
| M8-F | Pointer drives focus so the keyboard follows the click. Focus is server-owned `ShellState::focus` (`FocusTarget` editor/panel/prompt), the authoritative keymap context (chord resolution + text routing read it every frame), mutated only via `focus_panel()`/`focus_editor()`/`enter_prompt_focus()`/`exit_prompt_focus()`. Today the M8 pointer commands mostly do NOT move focus, so clicking the editor/a tab/a tree row while another surface is focused leaves the keyboard on the old surface. Make "focus follows the click" a contract of the acted-on-surface commands, entirely server-side (no app or protocol change; the app already re-adopts `shell.focus` each event): in `bind_selection`, when the command enum is `cursor_set_position` or `select_set_range`, call `shell.focus_editor()` on success; in `bind_tab`, `tab.activate` calls `shell.focus_editor()` on success; the `tree.select` handler calls `shell.focus_panel()` on success. **Safety is by enum/command gating, not by these being keyboard-unreachable:** the catalog marks `cursor.set_position`/`select.set_range`/`tab.activate` `keymap:true`/`palette:true`, but (a) NO keymap binding dispatches them — keyboard caret motion is a *different* `SelectionCommand` (`cursor_left`, `select_line_down`, …) and keyboard tab switching is `tab.next`/`tab.previous`, so ordinary keyboard use never triggers the focus move; and (b) when one IS reached another way (e.g. executing "Cursor Set Position" / "Tab Activate" from the palette — `cursor.set_position` is a palette candidate), focusing the surface the command acts on is the *correct* outcome anyway, so there is no wrong-focus hazard. `tree.select` is not a palette/keymap command. Net click outcomes: click editor → editor; click tab → editor; click a tree directory → panel (`tree.select` focuses panel, `tree.activate` toggle leaves it); click a tree file → editor when the open succeeds (`tree.select` focuses panel, then `tree.activate`'s file-open `focus_editor()` wins). **Failure interleave:** the click pair `[tree.select, tree.activate]` is two dispatches/transactions; if `tree.select` succeeds (focus → panel) but `tree.activate` fails (the file did not open), focus stays on the panel — acceptable, since nothing opened and the keyboard should keep navigating the tree. | `src/runtime/editing.cpp` (`bind_selection`), `src/runtime/files.cpp` (`bind_tab`), `src/runtime/navigation.cpp` (`tree.select`), `tests/runtime/test_runtime_editing.cpp`, `tests/runtime/test_runtime_files.cpp`, `tests/runtime/test_runtime_navigation.cpp` | runtime: with focus on the panel, `cursor.set_position` (and `select.set_range`) moves `shell.focus` to `editor`; a keyboard-style `cursor.left`/`select.line_down` from panel-or-editor focus does NOT change focus (the negative guard against regression); `tab.activate` from panel focus moves focus to `editor`; `tree.select` from editor focus moves focus to `panel`, the click pair `[tree.select, tree.activate]` on a file ends with focus `editor` (open succeeds) while on a directory ends with focus `panel`, and if `tree.activate` fails after `tree.select` focus remains `panel`; end-to-end (PTY) with the files bar focused (`Escape O`), a click in the document makes the arrow keys drive the caret (not the tree), and clicking a tab focuses the editor |
...
- **Focus follows the pointer, server-owned (M8-F).** Keyboard focus is a single
  server-owned `FocusTarget` (`ShellState::focus`) and the authoritative keymap
  context; it is never client state. M8-F does not add a client focus concept or a
  new focus command — it makes the existing acted-on-surface commands
  (`cursor.set_position`, `select.set_range`, `tab.activate`, `tree.select`) move
  that one focus value as part of their transaction, so the keyboard's active
  surface tracks the last click. The change is gated on the specific command/enum,
  so keyboard caret motion (`cursor_left`, `select_line_down`, …) and keyboard tab
  switching (`tab.next`/`previous`) — different commands — never move focus; the
  keyboard's own focus writers (`panel.focus`, `panel.toggle`, `pane.focus_*`,
  prompt open/close) remain the only others. Focus stays consolidated in one value.
