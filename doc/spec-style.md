...
Delivered: `Style` is a published snapshot section that the runtime owns and
both the shell layout and the renderer read, plus a `style.define` init-script
command that restyles a live session. The chrome-glyph inventory and the
scrollbar-metrics convention are recorded in Current state and Design above;
the reasoning behind the testing approach is in Considerations.
