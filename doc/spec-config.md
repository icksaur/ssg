...
| 7 | Add a runtime-string-sequence-parsing helper (space-separated stroke tokens, each via `KeyCodec::parseStroke`) and `keymap.bind`/`keymap.unbind` commands (descriptor + handler), reusing `KeymapMatcher::validate`/`hasGlobalBinding` for all-or-nothing per-call validation against `EditorRuntime::Impl::keymap` -- `keymap.unbind` re-checks the `settings.open` K6 escape hatch too, since removing the LAST such binding is exactly the case that breaks it (unlike K1/K2, which a removal can never violate); P0 catalog wiring identical to step 3's pattern, with `keymap.bind`/`keymap.unbind` themselves marked `keymap:false`/`palette:false` (each requires a typed payload no bare keystroke or parameterless palette invocation can supply, same shape as `find.update_query`/`tree.select`). K5 is NOT enforced via `data/required-commands.json`'s `keymap` boolean as originally planned: that flag does not distinguish argument-free commands from argument-required ones (e.g. `settings.set`/`cursor.set_position`/`text.insert` are all marked `keymap:true` despite requiring an argument) -- it only excludes commands unreachable for POINTER/CLIENT-fulfillment reasons. `keymap.bind` therefore accepts binding TO any known-or-unknown command id; binding to an argument-required one resolves at runtime to an empty-payload dispatch, which every existing handler already rejects gracefully (a "requires a typed ... payload" failure), not a crash -- though the keystroke-dispatch call site already discards every `CommandResult` (a pre-existing pattern), so that failure is silent. See `doc/features/keymap.md`'s Considerations. | `include/ssg/Keymap.h`/`src/Keymap.cpp` (sequence-string parser, `applyKeymapBind`/`applyKeymapUnbind`), `src/runtime/presentation.cpp`, `src/EditorSessionBuilder.cpp`, `data/required-commands.json` | test: bind/unbind add/replace/no-op/rejection cases against `KeymapViewState` equality (incl. unbind rejecting removal of the sole `settings.open` global binding); catalog-parity test still passes | K1, K2, K6 |
