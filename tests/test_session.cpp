...
ssg::InvocationPrincipal principal(
    std::uint64_t id,
    ssg::InvocationOrigin origin = ssg::InvocationOrigin::in_process,
...
TEST(total_order_and_registered_dispatch) {
    ...
    ASSERT_EQ(unknown.error, ssg::CommandError::unknown_command);
...
TEST(stale_rejection_applies_only_to_mutations) {
    ...
    ASSERT_EQ(stale_mutation.error, ssg::CommandError::stale_revision);
...
TEST(client_identity_and_principal_are_isolated) {
    ...
    ASSERT_EQ(unattached.error, ssg::CommandError::unknown_client);
}
...
TEST(handler_failure_is_atomic) {
    ...
    ASSERT_EQ(rejected_result.error, ssg::CommandError::handler_failed);
    ASSERT_EQ(session.revision(), ssg::Revision{1});
    ...
    ASSERT_EQ(threw.error, ssg::CommandError::handler_failed);
    ASSERT_EQ(session.revision(), ssg::Revision{1});
...
TEST(principal_capability_enforcement_has_origin_parity) {
    ...
    ASSERT_TRUE(session.attach(
        principal(1, ssg::InvocationOrigin::in_process,
                  {ssg::CapabilityId{"local_file_drop"}}),
    ...
    ASSERT_TRUE(session.attach(
        principal(3, ssg::InvocationOrigin::in_process), ssg::ViewId{3})
                    .accepted());
    ASSERT_TRUE(session.attach(
    ...
    ASSERT_EQ(
        ...
        ssg::CommandError::capability_denied);
    ASSERT_EQ(
