...
TEST(lowersLiteralAndProviderWidgets) {
    RowDescriptor row;
    row.left.push_back(literal(WidgetKind::Label, "l", "AB"));
    row.left.push_back(providerField("branch", "branch", 0));
    ...
    lowerChromeRow(row, {0, 0, 80, 1}, ShellNodeKind::HeaderField,
...
TEST(lowersCheckboxWithRoleOverride) {
    ...
    cb.role = "footer";
    RowDescriptor row;
    row.left.push_back(cb);

    std::vector<AccessibilityNode> out;
    lowerChromeRow(row, {3, 4, 80, 1}, ShellNodeKind::FooterField,
...
TEST(dropsEmptyProviderWidget) {
    RowDescriptor row;
    row.left.push_back(providerField("a", "missing", 0));
    row.left.push_back(literal(WidgetKind::Field, "b", "keep"));

    std::vector<AccessibilityNode> out;
    lowerChromeRow(row, {0, 0, 80, 1}, ShellNodeKind::HeaderField,
                   SemanticRole::Header, defaultStyle(), resolverFrom({}), out);
...
TEST(spacerCreatesGapWithoutANode) {
    RowDescriptor row;
    row.left.push_back(literal(WidgetKind::Field, "a", "x"));
    WidgetDescriptor spacer;
    spacer.kind = WidgetKind::Spacer;
    spacer.id = "s";
    spacer.width = 5;
    row.left.push_back(spacer);
    row.left.push_back(literal(WidgetKind::Field, "b", "y"));
...
TEST(composedProviderHeaderMatchesBuiltinSpanForSpan) {
    ...
    RowDescriptor row;
    row.separator = 1;
    row.left.push_back(providerField("path", "path", 1));
    row.left.push_back(providerField("git_branch", "git_branch", 5));
    const auto resolver = resolverFrom(
    ...
    lowerChromeRow(row, {header.x, header.y, header.width, 1},
                   ShellNodeKind::HeaderField, SemanticRole::Header, req.style,
                   resolver, composed);
...
TEST(dropsProviderWithEmptyLabel) {
    RowDescriptor row;
    row.left.push_back(providerField("a", "labelless", 0));
    row.left.push_back(literal(WidgetKind::Field, "b", "keep"));
