...
constexpr std::array<std::string_view, ssg::kSemanticRoleCount> kSemanticNames{
...
ssg::ThemeSetArguments fullIdentityTable(ssg::ThemeSnapshot const& theme) {
    ...
    for (const auto role : ssg::kAllSemanticRoles) {
        arguments.colors.emplace(
            std::string{kSemanticNames[static_cast<std::size_t>(role)]},
