...
struct LegacyUiFrameChanges {
    std::optional<UiSchema> schema;
    std::optional<UiStateSection> state;
    std::optional<UiPresenceSection> presence;
};

using UiFrameDeltaBody =
    std::variant<UiFrameReplacement, UiFrameChanges, LegacyUiFrameChanges>;
...
private:
    friend class ProtocolCodec;

    [[nodiscard]] static UiFrameDelta legacyChanges(
        LegacyUiFrameChanges changes);
