...
| Completed | Remove custom chrome composition | Deleted `ssg.chrome`, its decoder and composition plumbing. Core now publishes fixed semantic header/footer `UiNode` subtrees, while terminal row packing and glyph projection remain private TUI implementation details behind neutral UI-region APIs. |
| Completed | Separate UI-VM from grid helpers | `Widget.h` now contains only medium-neutral widget vocabulary. Terminal row fitting, cell measurement, glyph/text composition, input-line layout, and `WidgetStack` live in the TUI's `WidgetLayout`; `ViewSurface` remains the required opaque data-channel identity. |
