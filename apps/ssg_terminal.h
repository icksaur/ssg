...
// Which authoritative-snapshot axis an input event consumes, for the host's
// per-drain snapshot coalescing (Lever 3). A key/paste routes by the interaction
// routing state; a pointer/wheel hit-tests published geometry; the remaining
// kinds consume neither. The host refreshes before an event only when the axis
// it consumes was dirtied since the last snapshot, so a burst that dirties only
// the axis it does NOT consume coalesces to one snapshot.
struct ConsumedAxes {
    bool routing = false;
    bool geometry = false;
};

[[nodiscard]] constexpr ConsumedAxes consumed_axes(DecodeStatus status) noexcept {
    switch (status) {
...
// The per-drain snapshot-coalescing state (Lever 3). The host takes a fresh
// snapshot before an event only when the axis that event consumes has been
// dirtied since the last snapshot, so a burst that dirties only the axis it does
// NOT consume (a run of cursor-move keys dirties geometry, keys consume routing)
// collapses to one snapshot. A full refresh clears both axes. Extracted from the
// main loop so the seam is unit-testable: the loop wires dispatch effects and
// host-local picker mutations in, and asks needsRefresh() before each event.
class SnapshotCoalescer {
public:
    // Whether an event consuming `axes` needs a fresh snapshot before handling.
    [[nodiscard]] bool needsRefresh(ConsumedAxes axes) const noexcept {
        return (axes.routing && routingDirty_) ||
               (axes.geometry && geometryDirty_);
    }
    // A full snapshot was just taken: nothing is dirty until the next mutation.
    void noteRefreshed() noexcept {
        routingDirty_ = false;
        geometryDirty_ = false;
    }
    // A dispatch's authoritative effects: union them into the accumulator.
    void noteEffects(DispatchEffects effects) noexcept {
        routingDirty_ |= effects.routingChanged;
        geometryDirty_ |= effects.geometryChanged;
    }
    // A host-local picker mutation changes the PaletteReport the next snapshot is
    // built from, so it dirties both axes.
    void markPickerDirty() noexcept {
        routingDirty_ = true;
        geometryDirty_ = true;
    }
    [[nodiscard]] bool routingDirty() const noexcept { return routingDirty_; }
    [[nodiscard]] bool geometryDirty() const noexcept { return geometryDirty_; }

private:
    bool routingDirty_ = false;
