...
ShellLayoutResult computeShellLayout(const ShellLayoutRequest& request,
    ...
    if (!distractionFree) {
        ...
        // is placed.
        // A tab draws as leftEdge + title[+dirtySuffix] + rightEdge; its width is
        // measured from that, with no fixed padding.  Consecutive tabs are parted
        // by the separator glyph, emitted as its own non-interactive node.
        ...
        for (std::size_t i = firstTab; i < request.tabs.size(); ++i) {
            ...
            // tab it precedes and emitted only once that tab is confirmed to fit.
            // Keying it off the tab that actually lands -- rather than the logical
            // next index -- is what keeps a separator from dangling past the last
            // visible tab when the row runs out of room.
