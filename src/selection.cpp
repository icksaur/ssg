...
class TextModel {
public:
    TextModel(std::string_view text, int tab_width) : text_(text) {
        std::size_t start = 0;
        while (true) {
            const auto newline = text.find('\n', start);
            const auto end =
                newline == std::string_view::npos ? text.size() : newline;
            auto run = compute_cell_run(text.substr(start, end - start),
                                        tab_width);
            ...
            boundaries.push_back(DocumentPosition{
                ByteOffset{start}, LineIndex{lines_.size()}, CellIndex{0}});
            for (const auto& span : run.spans) {
                cell += span.cell_width;
                boundaries.push_back(DocumentPosition{
                    ByteOffset{start + span.byte_offset + span.byte_len},
                    LineIndex{lines_.size()}, CellIndex{cell}});
            }

            for (const auto& boundary : boundaries) {
                positions_.push_back(boundary);
            }
            lines_.push_back(
                LogicalLine{start, end, std::move(boundaries)});
    ...
    [[nodiscard]] std::optional<DocumentPosition> resolve(
        ...
        const auto found = std::lower_bound(
            positions_.begin(), positions_.end(), raw,
    ...
    [[nodiscard]] DocumentPosition vertical(
        ...
        if (down) {
            target = line_count > lines_.size() - 1 - current
                         ? lines_.size() - 1
                         : current + line_count;
            if (target == current) {
                return lines_[current].boundaries.back();
            }
        } else {
            target = line_count > current ? 0 : current - line_count;
            if (target == current) {
                return lines_[current].boundaries.front();
            }
        }
    ...
    [[nodiscard]] DocumentPosition word_left(
        const DocumentPosition& position) const {
        auto index = position_index(position);
        if (index == 0) {
            return positions_.front();
        }
        --index;
        const auto category = segment_category(index);
        if (category == SegmentCategory::word ||
            category == SegmentCategory::space) {
            while (index > 0 && segment_category(index - 1) == category) {
                --index;
            }
        }
    ...
    [[nodiscard]] DocumentPosition word_right(
        const DocumentPosition& position) const {
        auto index = position_index(position);
        if (index + 1 >= positions_.size()) {
            return positions_.back();
        }
        const auto category = segment_category(index);
        do {
            ++index;
        } while (index + 1 < positions_.size() &&
                 segment_category(index) == category);
        return positions_[index];
    }
    ...
    [[nodiscard]] ViewportViewState viewport_state(
        ...
        return compute_viewport(runs_, dimensions,
...
private:
    ...
    [[nodiscard]] SegmentCategory segment_category(
        std::size_t index) const {
        const auto start =
            static_cast<std::size_t>(positions_[index].byte_offset.value());
    ...
    [[nodiscard]] DocumentPosition position_for_cell(
        ...
        const auto& boundaries = lines_[line_index].boundaries;
    ...
    [[nodiscard]] std::vector<WrappedRow> wrapped_rows(
        ...
        for (std::size_t line_index = 0; line_index < lines_.size();
             ++line_index) {
            const auto& line = lines_[line_index];
            const auto& run = runs_[line_index];
            if (run.spans.empty()) {
    ...
    [[nodiscard]] DocumentPosition position_for_wrapped_cell(
        ...
        const auto target =
            std::min(row.start_cell + desired_cell.value(),
                     row.end_cell);
        const auto& boundaries = lines_[row.logical_line].boundaries;
    ...
    std::vector<DocumentPosition> positions_;
};
...
SelectionNavigationResult apply_selection_navigation(
    ...
    const auto current_viewport =
        model.viewport_state(viewport, before.first_visual_row);
